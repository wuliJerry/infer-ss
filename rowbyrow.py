...
def solve_lp(config:CostModelConfig, 
    ...
    prob += dtogp == (1 / dtog_bdw) * (wn * wi + 2 * hn * s * h1 * bls)
    prob += gtodp == (1 / gtod_bdw_cache_p) * (4 * cn * bls * (s + 1) * h1) + 1 / (gtod_bdw_hidden_p) * (2 * hn * s * h1 * bls)
    ...
    prob += gtodg == (1 / dtog_bdw) * (4 * cn * bls * h1 + 2 * hn * h1 * bls)
    prob += dtogg == (1 / gtod_bdw_g) * (4 * cn * bls * (s + n / 2) * h1 \
                                        + wn * wi + \
                                        2 * hn * h1 * bls)
    prob += compg == (1 / mm_flops_g) * bls * (wi) \
