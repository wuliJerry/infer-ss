...
def solve_lp(config:CostModelConfig, 
    ...
    # Memory requirements for other matrices without sparsity (forward and backward)
    d = h1 / nh
    Mem_others = n * (d + 2)
    Mem_others_back = 3 * n * d + 2 * n

    # Adjust memory requirements based on the sparsity factor
    Mem_total_sparse_forward = l * (Sparsity_Factor * Mem_QKV + inter_mem + Mem_others)
    Mem_total_sparse_backward = l * (Sparsity_Factor * Mem_QKVO + inter_mem + Mem_others_back)
    prob += cache_peak >= Mem_total_sparse_forward
    prob += cache_peak >= Mem_total_sparse_backward
    prob += cache_peak <= cache
